...
export default function EditStaffPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchStaff = async () => {
      try {
        ...
        showAlert('Error', 'Could not fetch staff details.', 'error');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showAlert('Error', err.message || 'Failed to update staff member.', 'error');
      setLoading(false);
    }
  };

  if (initialLoading) {
      return <div style={{ padding: '2rem' }}>Loading staff details...</div>;
  }
            ...
            <div style={{ border: '2px dashed #ddd', padding: '2rem', borderRadius: '6px', textAlign: 'center', cursor: 'pointer', position: 'relative' }}>
               ...
               <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                    {(previewUrl || existingImageUrl) ? (
                        <img src={previewUrl || existingImageUrl!} alt="Preview" style={{ width: '120px', height: '120px', objectFit: 'cover', borderRadius: '50%', marginBottom: '1rem' }} />
                    ) : (
                        <Upload size={32} style={{ marginBottom: '0.5rem', color: '#9ca3af' }} />
                    )}
                   {previewUrl ? <span style={{ color: '#2563eb' }}>Click to change (New selected)</span> : (
                       existingImageUrl ? <span style={{ color: '#2563eb' }}>Click to change current image</span> : <span>Click or drag image to upload</span>
                   )}
               </div>
            </div>
          </div>
