...
export default function StaffLayout({
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      // Allow Admin or Staff to access this dashboard
      if (error || (profile?.usertype !== 'admin' && profile?.usertype !== 'staff')) {
  ...
  if (loading) {
    return <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh' }}>Verifying Staff Access...</div>;
  }
  ...
  const navItems = [
    ...
    { name: 'Products', href: '/staff/products', icon: <Package size={20} /> },
    { name: 'Orders', href: '/staff/orders', icon: <ShoppingCart size={20} /> },
  ];
  ...
  return (
    <div className={styles.adminContainer}>
      ...
      <div className={styles.mobileHeader}>
          ...
          <span style={{ fontWeight: 'bold' }}>Staff Panel</span>
      ...
      <aside className={`${styles.sidebar} ${sidebarOpen ? styles.sidebarOpen : ''}`}>
        <div className={styles.sidebarHeader}>
          <div className={styles.brand}>Staff Panel</div>
