...
interface Order {
  ...
  quantity: number;
  user_id: string;
  user_email?: string; // We might need to fetch this separately or via join
}
...
export default function AdminOrdersPage() {
  ...
  const fetchOrders = async () => {
    ...
    if (userIds.length > 0) {
        ...
        if (profilesData) {
            profilesData.forEach((p: any) => {
                profilesMap[p.id] = p.email || p.full_name || 'Unknown User';
            });
        }
    }
    ...
    const formattedOrders = ordersData.map((order: any) => ({
        ...order,
        user_email: profilesMap[order.user_id] || 'N/A'
    }));
                ...
                {filteredOrders.map(order => (
                  <tr key={order.id} style={{ borderBottom: '1px solid #eee' }}>
                    ...
                    <td style={{ padding: '1rem' }}>
                      <div style={{ fontSize: '0.95rem' }}>{order.user_email}</div>
                      <div style={{ fontSize: '0.8rem', color: '#888' }}>{order.user_id.slice(0,8)}...</div>
                    </td>
                    <td style={{ padding: '1rem' }}>
